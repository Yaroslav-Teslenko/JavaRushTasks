...
Метод addTo(String, String[]) додає до рядка, отриманого як перший параметр, усі рядки за порядком із
масиву рядків, отриманого як другий параметр, і повертає об'єкт типу StringBuilder на основі цього рядка.
Щоб додати нове значення до наявного об'єкта типу StringBuilder, потрібно викликати його метод append(String)
і передати в нього це нове значення.
...
Щоб замінити частину значення об'єкта типу StringBuilder, потрібно викликати його метод replace (int, int, String)
і передати в нього початкову та кінцеву позиції для заміни, а також нове значення.
