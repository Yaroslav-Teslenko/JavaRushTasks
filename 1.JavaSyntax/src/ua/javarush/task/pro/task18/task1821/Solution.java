...
метод getCheaperCar(Stream<Car>, Car) в іншому списку шукає автомобіль дешевший, ніж отриманий у методі getCheapestCar(Stream<Car>).
Методи повертають не сам автомобіль (об'єкт типу Car), а об'єкт типу Optional<Car>.
Підказка:
Для пошуку елемента з мінімальним шуканим значенням у потоці даних використовуй метод min(), а щоб знайти перший-ліпший відповідний елемент із потоку відфільтрованих даних, використовуй метод findFirst().
