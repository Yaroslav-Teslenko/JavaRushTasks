...
//Студент (клас Student) має ім'я (поле name) і вік (поле age).
// У програмі потрібно відсортувати студентів за віком у порядку
// спадання (від найстаршого до наймолодшого).
//Для цього використовується метод
// Collections.sort(ArrayList<Student>, Comparator<Student>), в який передається
// список студентів students і компаратор сортування за спаданням значень віку.
//
//Твоє завдання — реалізувати цей компаратор. Для цього в класі
// AgeComparator слід імплементувати інтерфейс
// Comparator<Student> і реалізувати метод
// int compare(Student student1, Student student2), який повертає числове значення згідно з такими умовами:
//
